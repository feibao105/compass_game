...
def point_line_dist(p,p1,p2):
    #点p到直线p1p2的距离(面积法:叉积÷底边长)
    lx=p2[0]-p1[0]
    ly=p2[1]-p1[1]
    line_len=distance(p1,p2)
    if line_len<1:
        return 9999
    cross=abs(lx*(p[1]-p1[1])-ly*(p[0]-p1[0]))
    return cross/line_len
def project_t(p,p1,p2):
    #点p投影到线段p1p2上的位置参数:
    #0~1之间=落在线段上；小于0或大于1=落在延长线上
    lx=p2[0]-p1[0]
    ly=p2[1]-p1[1]
    line_len=distance(p1,p2)
    if line_len<1:
        return -999
    return ((p[0]-p1[0])*lx+(p[1]-p1[1])*ly)/(line_len*line_len)
...
def angle_bisector(vertex, p1, p2, length=2000):
    ...
    if d_len<0.0001:
        #两边恰好反向（180°平角）：角平分线=其中一边旋转90°，也就是垂线
        dx,dy=-u1y,u1x
        d_len=1
...
#垂线工具
def perpendicular_through_point(p,l1,l2,length=2000):
    #过点p作直线l1l2的垂线，返回延长后的两端点
    dx=l2[0]-l1[0]
    dy=l2[1]-l1[1]
    #垂线方向=(-dy,dx)（方向旋转90°）
    px,py=-dy,dx
    ...
    return ((p[0]-px*length,p[1]-py*length),(p[0]+px*length,p[1]+py*length))

def nearest_line(pos,lines,threshold=15):
    #找离鼠标最近的那条线（点到线距离<t且投影在线段上）
    best=None
    bd=threshold
    for q in lines:
        d=point_line_dist(pos,q[0],q[1])
        t=project_t(pos,q[0],q[1])
        if -0.05<=t<=1.05 and d<bd:
            bd=d
            best=q
    return best
...
#吸附工具
def snap_to_point(pos,points,threshold=25):
    #把鼠标位置吸附到最近的已有点
    if not points:
        return pos
