...
def reset_level():
    ...
    last=None
    log.info("重新开始关卡")
def draw_toolbar(screen,font,current_tool,tool4_unlocked):
    # 画工具栏背景
...
def check_toolbar_click(pos,tool4_unlocked):
    total_width=len(tools)*100+(len(tools)-1)*10
    start_x=(800-total_width)//2
    button_y=640+20//2
    for i,tool in enumerate(tools):
...
#主函数
async def main():
    global running,screen,font,clock,log
    #创建一个窗口
    py.init()
    font=load_font(24)
    screen=py.display.set_mode((800,700))
    py.display.set_caption("尺规作图2.0")
    log=lo.setup_logger()
    clock=py.time.Clock()
    start_time=time.time()
    global number,last,lines,circles,points,history
    global passed,pass_time,score,grade,tool4
    ...
    while running:
        ...
        for i in py.event.get():
            if i.type==py.QUIT:
                ...
                if i.button==1:
                    clicked=check_toolbar_click(i.pos, tool4)
        ...
        draw_toolbar(screen,font,number,tool4)
        py.display.flip()
        #控制帧率
        clock.tick(60)
        #pygbag必须
