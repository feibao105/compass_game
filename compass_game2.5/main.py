...
async def main():
    ...
    py.display.set_caption("尺规作图2.5")
    log=lo.setup_logger()
    clock=py.time.Clock()
    start_time=time.time()
    ...
    global current_level,level,check,selecting
    while running:
            ...
            if i.type==py.QUIT:
                ...
                if i.button==1:
                    if selecting:
                        ...
                        r=ui.check_level_select_click(i.pos,levels,scores)
                        if r=="close":
                            selecting=False
                        elif r is not None:
                            current_level=r
        ...
        if selecting:
            ui.draw_level_select(screen,levels,sa.load_scores(),current_level)
