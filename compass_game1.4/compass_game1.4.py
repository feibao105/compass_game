...
py.display.set_caption("尺规作图1.4")
__savefile__="scores.json"
log=lo.setup_logger()
clock=py.time.Clock()
...
def reset_level():
    global start_time,passed,last,score,grade
    lines.clear()
    circles.clear()
    points.clear()
    history.clear()
...
def load_scores():
    ...
    with open(__savefile__,"r",encoding="utf-8") as f:
        return json.load(f)
def save_scores(level_num,steps,elements,elapsed,grade):
    scores=load_scores()
    key=str(level_num)
...
while running:
    ...
    if not passed:
        if check(points,lines,circles,level["given_points"]):
            ...
            save_scores(current_level+1,step,e,int(pass_time),grade)
